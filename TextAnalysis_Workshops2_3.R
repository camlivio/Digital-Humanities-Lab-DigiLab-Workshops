...
########################################
########## shoutout to the terrific tutorial by Dr. Schweinberger
########################################
...
#Focus: sentiment analysis & tf-idf

#sentiment analysis with Tidytext: the analysis is based on unigrams only, 
#so qualifiers are not taken into consideration (.e.g.,"no good") 

#### last week we created this object:
bing_word_counts
...
### visualize sentiment by type now 
bing_word_counts %>% 
  group_by(sentiment) %>% 
  mutate(sentiment = reorder(sentiment, n)) %>%
  ggplot(aes(n, sentiment, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~type, scales = "free_y") +
  labs(x = "Distribution of Sentiment by Show Type",
       y = NULL)

### nice! what else can we do with it?

###############
### Shoutout to Dr. Adriana Picoral's blog for the terrific code:
###############

net_sentiment <- clean_net %>%
...
word_sentiment_count <- net_sentiment %>%
  count(word, sentiment) 

#### now calculate tf-idf
## term frequency inverse document frequency (tf-idf): 
## show Silge and Robinson's chapter 3

## counting instances of each word per sentiment
## then we calculate tf-idf based on n
...
sentiment_tf_idf %>%
  ...
  labs(y = "") +
  ggtitle("Top 15 most important words per sentiment",
          subtitle = "labels show -n") +
  theme_bw()
